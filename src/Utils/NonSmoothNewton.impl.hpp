...
struct LinearSolver
{
  ...
  static ReturnType solve( const typename Traits::Matrix &A, const typename Traits::Vector &b ) 
  {
...
typename NonSmoothNewton< NSFunction >::Scalar NonSmoothNewton<NSFunction>::solve(
  Vector& x ) const
{
  static const Scalar sigma2 = 1.e-6 ;
  ...
  for( unsigned iter = 0 ; iter < m_maxIters ; ++iter )
  {
    ...
    //if( !NumTraits< Scalar >::isZero( dF_dx.determinant() ) )
    //{
